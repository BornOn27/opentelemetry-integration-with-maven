package com.livspace.otel.extension;
...
public class Customizer implements AutoConfigurationCustomizerProvider {
  ...
  public void customize(AutoConfigurationCustomizer autoConfiguration) {
    ...
    String traceRatio = System.getenv("OTEL_TRACES_SAMPLER_ARG") ;
    ...
    autoConfiguration.addTracerProviderCustomizer(
        (sdkTracerProviderBuilder, configProperties) ->
            sdkTracerProviderBuilder.setSampler(
                Sampler.parentBased(
                    RuleBasedRoutingSampler.builder(SpanKind.SERVER, Sampler.traceIdRatioBased(StringUtils.isNullOrEmpty(traceRatio) ? 0.1 : Double.parseDouble(traceRatio)))
                        .drop(SemanticAttributes.HTTP_TARGET, "/health-check")
                        .build())));
  }
}
